...
class Tile(pg.sprite.Sprite):
    ...
    short_tile = pg.image.load("Resourses/Images/short_tile.png")
    short_tile = pg.transform.scale(short_tile, (SIZE[0] // 4, SIZE[1] // 3.5))
    short_tile_pressed = pg.image.load("Resourses/Images/short_tile_pressed.png")
    short_tile_pressed = pg.transform.scale(short_tile_pressed, (SIZE[0] // 4, SIZE[1] // 3.5))

    long_tile = pg.image.load("Resourses/Images/long_tile.png")
    long_tile = pg.transform.scale(long_tile, (SIZE[0] // 4, SIZE[1] // 2.2))
    long_tile_pressed = pg.image.load("Resourses/Images/long_tile_pressed.png")
    ...
    def update(self):
        ...
        if pg.mouse.get_pressed()[0] and self.rect.collidepoint(mouse_pos):
            self.press()
...
class Song:
    ...
    def __init__(self, name, notes, duration, rect, interval=0.5):
        ...
        self.text = f1.render(self.name, True, pg.Color(self.color))
...
def play_note():
    ...
    sound = pg.mixer.Sound('Resourses/Sounds/Notes/' + playing_song.notes[next_note] + '.ogg')
    sound.play()
    next_note += 1
...
winSound = pg.mixer.Sound('Resourses/Sounds/inGame/win.mp3')
looseSound = pg.mixer.Sound('Resourses/Sounds/inGame/loose.mp3')

background_menu = pg.image.load("Resourses/Images/menu.png")
    ...
    if mode == "menu":
        ...
        for song in Song.songs:
            if song.rect.collidepoint(mouse_pos):
                ...
                song.color = 'red'
            song.text = f1.render(song.name, True, pg.Color(song.color))
        ...
        y = 1
        for song in Song.songs:
            pg.draw.rect(screen, pg.Color(song.color), song.rect, 5)
